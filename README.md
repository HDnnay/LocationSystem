# LocationSystem - 牙科诊所管理系统

## 项目概述

LocationSystem 是一个现代化的牙科诊所管理系统，提供全面的牙科诊所运营管理功能，包括牙医管理、患者管理、预约管理、角色权限管理等。系统采用前后端分离架构，后端基于 ASP.NET Core 构建，前端使用 Vue 框架，支持实时通信和事件驱动的缓存更新机制。

## 功能特性

### 核心功能

- **用户认证与授权**：基于 JWT 的认证系统，支持牙医和患者两种用户类型
- **牙医管理**：添加、编辑、删除牙医信息，分配角色和权限
- **患者管理**：患者信息录入、编辑、查询，支持患者历史记录管理
- **牙科诊所管理**：诊所信息管理，包括地址、联系方式等
- **预约管理**：预约创建、编辑、删除，支持日历视图
...
- **IEventBus 简单实现**：轻量级基于内存的事件总线实现，支持事件的发布和订阅
- **事件驱动架构**：基于内存事件总线的事件发布/订阅模式
...
- 问题反馈：[GitHub Issues](https://github.com/yourusername/LocationSystem/issues)
