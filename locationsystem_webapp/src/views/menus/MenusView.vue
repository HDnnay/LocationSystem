...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const handlerSelectPermissions = (permissions) => {
        const selected = [];
      ...
      const handleAssignPermissions = async (menu) => {
        ...
        const api = await import('@/api')
        const response = await api.default.permissions.getMeuPermissionTreeWithCheck(menu.id)
        console.log("权限树：");
        console.log(response);

        permissionTree.value = response
        checkedPermissionIds.value = handlerSelectPermissions(response)
